...
Atelier de packaging de couverture pour l'auto-édition : l'app est un seul fichier HTML autonome, sans build (trois onglets — 1ère, 4ème, Assemblage — et export PDF de la planche). `outils/` conserve la chaîne Python (pandoc + WeasyPrint) qui composait l'intérieur avant le passage à Typst : c'est de l'archive, plus maintenue — l'app fait foi ; `build/` n'est jamais tracké : ressources partagées dans `build/in/{covers,texts,editors}/`, un répertoire de travail par combinaison texte × couverture (au minimum un `livre.toml`, dont les chemins partent de `build/`), sorties dans son `out/` et packages éditeur dans `out/<éditeur>/`. Le `livre.toml` fait foi pour l'identité du livre ; le titre et l'auteur embarqués dans le PNG sont un rendu, jamais une source — aucun script ne lit ce chunk.
...
- `cargo run --example temoin` si un fichier de `app/src-tauri/` a changé : le compte de pages affiché est le témoin de non-régression, à comparer au précédent sur le même manuscrit.
