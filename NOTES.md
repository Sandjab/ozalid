...
**`enregistrerQuelquePart` et `enregistrerSous` (`app/src/app.js`) dupliquent un `try/catch` qui écrit dans `#etat`**, au lieu de passer par `tente`, qui existe précisément pour cela et qui sait désormais choisir sa cible selon que la section porteuse est masquée ou non. Le doublon date d'avant le cycle de vie du projet ; il n'a pas été touché pour ne pas mêler un refactor à ce chantier-là.
